...
from aphelion.core.execution import apply_enforcement, build_trade_proposal, compute_commission
...
class BacktestEngine:
    ...
    def __init__(
        ...
        self._feature_engine: Optional[LegacyFeatureEngine] = None
    ...
    def _reset_runtime_state(self) -> None:
        ...
        if self._config.enable_feature_engine:
            self._feature_engine = LegacyFeatureEngine(self._data_layer)
        else:
    ...
    def _apply_execution_enforcement(self, order: Order, bar: Bar) -> Optional[Order]:
        ...
        Delegates to the shared ``apply_enforcement`` helper so that backtest
        and live/paper paths behave identically.
        """
        if order.order_type == OrderType.MARKET:
        ...
        return apply_enforcement(order, proposed_entry, self._enforcer)

...
async def _run_paper_from_args(args: argparse.Namespace) -> int:
    logger = get_logger("aphelion.run_paper")
    ...
    try:
        ...
        logger.info("paper_run_summary", summary=result.summary())
        return 0
...
def _start_training_subprocess(train_full: bool, symbol: str = "XAUUSD") -> subprocess.Popen:
    ...
    if not data_path:
        raise FileNotFoundError(
            f"No real data found for {symbol}. Build bars/features with mt5pipe first, or place a prepared dataset under data/processed/{symbol}."
        )
...
async def _run_demo_from_args(args: argparse.Namespace) -> int:
    logger = get_logger("aphelion.runall")

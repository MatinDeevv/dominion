...
class PerformanceValidator:
    ...
    def validate(
        ...
        results.append(ValidationResult(
            "min_trades", trade_count >= TITAN_REQUIREMENTS["min_trades_for_significance"],
            float(trade_count), TITAN_REQUIREMENTS["min_trades_for_significance"],
        ))
...
class StressValidator:
    ...
    def validate(
        ...
        mc_p1_drawdown: Optional[float] = None,
        stress_scenarios_passed: Optional[int] = None,
        stress_scenarios_total: Optional[int] = None,
        mc_min_p5_sharpe: Optional[float] = None,
        ...
        sharpe_threshold = (
            TITAN_REQUIREMENTS["mc_5th_percentile_sharpe"]
            if mc_min_p5_sharpe is None
            else mc_min_p5_sharpe
        )
        drawdown_threshold = (
            TITAN_REQUIREMENTS["mc_95th_percentile_max_dd"]
            if mc_max_p1_drawdown is None
            else mc_max_p1_drawdown
        )

        if mc_sharpes is not None or mc_drawdowns is not None:
            p5_sharpe = float(np.percentile(mc_sharpes, 5)) if mc_sharpes else 0.0
            p95_dd = float(np.percentile(mc_drawdowns, 95)) if mc_drawdowns else 1.0
            results.append(ValidationResult(
                "mc_5th_percentile_sharpe",
                p5_sharpe >= sharpe_threshold,
                p5_sharpe, sharpe_threshold,
            ))
            results.append(ValidationResult(
                "mc_95th_percentile_max_dd",
                p95_dd <= drawdown_threshold,
                p95_dd, drawdown_threshold,
            ))
        else:
            p5_sharpe = float(mc_p5_sharpe or 0.0)
            p1_dd = float(mc_p1_drawdown or 0.0)
            results.append(ValidationResult(
                "mc_p5_sharpe",
                p5_sharpe >= sharpe_threshold,
                p5_sharpe, sharpe_threshold,
                "Monte Carlo 5th percentile Sharpe",
            ))
            results.append(ValidationResult(
                "mc_p1_drawdown",
                p1_dd <= drawdown_threshold,
                p1_dd, drawdown_threshold,
                "Monte Carlo 1st percentile drawdown",
            ))
            if stress_scenarios_passed is not None and stress_scenarios_total is not None:
                pass_rate = (
                    stress_scenarios_passed / stress_scenarios_total
                    if stress_scenarios_total > 0
                    else 0.0
                )
                results.append(ValidationResult(
                    "stress_pass_rate",
                    pass_rate >= min_stress_pass_rate,
                    pass_rate, min_stress_pass_rate,
                    f"{stress_scenarios_passed}/{stress_scenarios_total} scenarios passed",
                ))
...
class RegressionValidator:
    ...
    def validate(
        ...
        if baseline_metrics is not None or current_metrics is not None:
            results = []
            for metric, baseline_val in (baseline_metrics or {}).items():
                current_val = (current_metrics or {}).get(metric, 0.0)
                if baseline_val == 0:
                    continue
        ...
        if baseline_sharpe and baseline_sharpe > 0:
            regression = ((current_sharpe or 0.0) - baseline_sharpe) / baseline_sharpe
        else:
            regression = 0.0
...
class LatencyValidator:
    ...
    def validate(
        ...
        latency_buckets: Optional[Dict[str, float]] = None,
        ...
        if latency_buckets is None:
            actual_latency = float(p99_latency_ms or 0.0)
            return [ValidationResult(
                "max_p99_latency_ms",
                actual_latency <= TITAN_REQUIREMENTS["max_p99_latency_ms"],
                actual_latency, TITAN_REQUIREMENTS["max_p99_latency_ms"],
            )]
        ...
        results = [ValidationResult(
            "total_pipeline_p99",
            total_p99 <= max_total_p99_ms,
            total_p99, max_total_p99_ms,
            "Sum of all pipeline stage p99 latencies",
        )]

        for operation, p99 in latency_buckets.items():
            results.append(ValidationResult(
                f"latency_{operation}",
                p99 <= max_single_p99_ms,
                p99, max_single_p99_ms,
            ))
        return results

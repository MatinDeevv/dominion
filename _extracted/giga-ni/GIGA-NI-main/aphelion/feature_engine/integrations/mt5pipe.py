...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    event_count = len(load_mt5pipe_events(config))
    logger.info("mt5pipe_replay_complete", replayed_events=event_count, emitted_snapshots=len(snapshots))
    if snapshots:
        latest = snapshots[-1]
        logger.info(
            "mt5pipe_replay_latest_snapshot",
            ts_event_ns=latest.ts_event_ns,

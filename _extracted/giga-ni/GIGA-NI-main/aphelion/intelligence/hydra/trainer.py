...
if HAS_TORCH:
    ...
    class LabelSmoothingFocalLoss(nn.Module):
        ...
        def __init__(self, gamma: float = 2.0, alpha: Optional[list[float]] = None,
                     smoothing: float = 0.1, n_classes: int = 3) -> None:
            super().__init__()
            self.gamma = gamma
    ...
    class HydraTrainer:
        ...
        def __init__(
            ...
            logger.info("hydra_trainer_initialized", parameters=n_params, device=str(self._device))
            ...
            if cfg.use_amp and self._device.type == "cuda":
                try:
                    if torch.cuda.is_bf16_supported():
                        self._use_bf16 = True
                        logger.info("hydra_bfloat16_enabled")
            ...
            if cfg.swa_start_epoch < cfg.max_epochs:
                try:
                    ...
                    logger.info("hydra_swa_enabled", start_epoch=cfg.swa_start_epoch)
                except ImportError:
                    pass
        ...
        def _mixup_data(
            ...
            y15m: torch.Tensor,
            y1h: torch.Tensor,
            raw_ret: torch.Tensor,
            alpha: float = 0.2,
        ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, float | None]:
            if alpha <= 0:
        ...
        def _compute_aux_losses(
            self, logits_list: list[torch.Tensor], targets: list[torch.Tensor]
        ) -> torch.Tensor:
        ...
        def train(self, train_loader: Any, val_loader: Any) -> dict[str, float | int]:
            cfg = self._config
            n_params = self._model.count_parameters()
            ...
            logger.info(
                ...
                amp_mode="BF16" if self._use_bf16 else "FP16" if cfg.use_amp else "OFF",
            ...
            for epoch in range(cfg.max_epochs):
                ...
                logger.info(
                    ...
                    accuracy=float(val_m["accuracy"]),
                    sharpe=float(sharpe),
                    mean_confidence=float(val_m.get("mean_confidence", 0.0)),
                    ...
                    patience_state=pat,
                    improved=improved,
                    best_marker=mark.strip() or None,
                    confidence_note=conf.strip() or None,
                    swa_tag=swa_tag.strip() or None,
                )

                # Periodic checkpoint
                if (epoch + 1) % cfg.save_every_n_epochs == 0:
                    self._save_checkpoint("latest")
                    logger.info("hydra_checkpoint_saved", epoch=epoch + 1, label="latest")

                # Early stopping
                if self._epochs_no_improve >= cfg.patience:
                    logger.info(
                        "hydra_early_stop_triggered",
                        epoch=epoch + 1,
                        patience=cfg.patience,
                        best_val_sharpe=float(self._best_val_sharpe),
            ...
            if self._swa_model is not None and self._epoch >= cfg.swa_start_epoch:
                try:
                    ...
                    logger.info("hydra_swa_bn_update_started")
                    update_bn(train_loader, self._swa_model, device=self._device)
                    self._save_checkpoint("swa_final")
                    logger.info("hydra_swa_finalized")
                except Exception as e:
                    logger.warning("hydra_swa_bn_update_failed", error=str(e))

            total_t = time.time() - t_start
            logger.info(
                "hydra_training_finished",
                total_time=self._format_eta(total_t),
        ...
        def _train_epoch(self, loader: Any) -> dict[str, float]:
            ...
            for bi, batch in enumerate(pbar):
                ...
                if torch.isnan(cont).any() or torch.isinf(cont).any():
                    if bi == 0:
                        logger.warning("hydra_invalid_input_batch_skipped", batch_index=bi)
                    continue
                ...
                if torch.isnan(loss) or torch.isinf(loss):
                    if bi < 3:
                        logger.warning("hydra_invalid_loss_batch_skipped", batch_index=bi)
                    self._optimizer.zero_grad(set_to_none=True)
                    n_batches += 1

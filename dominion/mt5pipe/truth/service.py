...
from mt5pipe.contracts.compiler import DatasetSpec, LineageManifest
...
class TruthService:
    ...
    def _source_quality_metrics(
        ...
        *,
        ...
        required_raw_date_set = set.union(*covered_by_broker.values()) if covered_by_broker else set()
        if not required_raw_date_set:
            required_raw_date_set = requested_date_set
        ...
        missing_dates = {
            broker_id: sorted(required_raw_date_set - covered_dates)
        ...
        metrics = {
            ...
            "required_raw_expected_days": len(required_raw_date_set),
            "requested_calendar_days": len(requested_date_set),
            "synchronized_raw_days": len(synchronized_dates),
            "synchronized_raw_coverage_ratio": (
                round(len(synchronized_dates) / len(required_raw_date_set), 6) if required_raw_date_set else 0.0
            ),

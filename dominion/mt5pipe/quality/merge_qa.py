...
def format_daily_merge_qa_summary(report_df: pl.DataFrame) -> str:
    ...
    def _int_cell(row: dict[str, object], key: str) -> int:
        value = row.get(key, 0)
        return int(value or 0)

    def _series_total(column: str) -> int:
        if column not in report_df.columns:
            return 0
        value = report_df[column].sum()
        return int(value or 0)

    symbol = report_df["symbol"][0]
    start_date = report_df["date"].min()
    end_date = report_df["date"].max()
    total_canonical = _series_total("canonical_rows")
    total_dual = _series_total("canonical_dual_rows")
    total_near = _series_total("near_miss_pairs")
    total_rejects = _series_total("validation_reject_count")
    total_conflicts = _series_total("conflicts")
    total_gap_events = _series_total("gaps_gt_1m")
    dual_ratio = (total_dual / total_canonical) if total_canonical else 0.0
    max_gap_value = report_df["max_gap_minutes"].max() if "max_gap_minutes" in report_df.columns else 0.0
    max_gap = float(max_gap_value or 0.0)
    ...
    for row in report_df.iter_rows(named=True):
        ...
        lines.append(
            f"{row['date']} "
            f"{_int_cell(row, 'broker_a_tick_count'):>9,} "
            f"{_int_cell(row, 'broker_b_tick_count'):>10,} "
            f"{_int_cell(row, 'canonical_rows'):>12,} "
            f"{_int_cell(row, 'canonical_dual_rows'):>11,} "
            f"{dual_pct:>7.2f} "
            f"{_int_cell(row, 'bucket_both'):>13,} "
            f"{_int_cell(row, 'near_miss_pairs'):>11,} "
            f"{_int_cell(row, 'validation_reject_count'):>9,} "
            f"{_int_cell(row, 'gaps_gt_1m'):>9,} "
            f"{float(row.get('max_gap_minutes') or 0.0):>10.2f}"
        )

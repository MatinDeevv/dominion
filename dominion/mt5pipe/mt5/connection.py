...
def _require_mt5() -> Any:
    if mt5 is None:
        raise MT5ConnectionError(
            "MetaTrader5 package is required for MT5 connectivity. "
...
class MT5Connection:
    ...
    def copy_ticks_range(
        ...
        result = mt5_mod.copy_ticks_range(broker_symbol, date_from, date_to, flags)
        if result is None:
            err = mt5_mod.last_error()
    ...
    def copy_rates_range(
        ...
        mt5_mod = _require_mt5()
        broker_symbol = self._cfg.resolve_symbol(symbol)
        result = mt5_mod.copy_rates_range(broker_symbol, timeframe, date_from, date_to)
        if result is None:
            err = mt5_mod.last_error()
    ...
    def symbol_info(self, symbol: str) -> Any:
        ...
        mt5_mod.symbol_select(broker_symbol, True)
        return mt5_mod.symbol_info(broker_symbol)

    def symbol_info_tick(self, symbol: str) -> Any:
        broker_symbol = self._cfg.resolve_symbol(symbol)
        return _require_mt5().symbol_info_tick(broker_symbol)

    def symbols_total(self) -> int:
        return _require_mt5().symbols_total() or 0

    def symbols_get(self, group: str = "") -> Any:
        mt5_mod = _require_mt5()
        if group:
            return mt5_mod.symbols_get(group=group)
        return mt5_mod.symbols_get()

    def account_info(self) -> Any:
        return _require_mt5().account_info()

    def terminal_info(self) -> Any:
        return _require_mt5().terminal_info()

    def orders_get(self, symbol: str = "") -> Any:
        mt5_mod = _require_mt5()
        if symbol:
            broker_symbol = self._cfg.resolve_symbol(symbol)
            return mt5_mod.orders_get(symbol=broker_symbol)
        return mt5_mod.orders_get()

    def positions_get(self, symbol: str = "") -> Any:
        mt5_mod = _require_mt5()
        if symbol:
            broker_symbol = self._cfg.resolve_symbol(symbol)
            return mt5_mod.positions_get(symbol=broker_symbol)
        return mt5_mod.positions_get()
    ...
    def history_orders_get(
        ...
        mt5_mod = _require_mt5()
        if group:
            return mt5_mod.history_orders_get(date_from, date_to, group=group)
        return mt5_mod.history_orders_get(date_from, date_to)
    ...
    def history_deals_get(
        ...
        mt5_mod = _require_mt5()
        if group:
            return mt5_mod.history_deals_get(date_from, date_to, group=group)
        return mt5_mod.history_deals_get(date_from, date_to)

    def market_book_add(self, symbol: str) -> bool:
        broker_symbol = self._cfg.resolve_symbol(symbol)
        return bool(_require_mt5().market_book_add(broker_symbol))

    def market_book_get(self, symbol: str) -> Any:
        broker_symbol = self._cfg.resolve_symbol(symbol)
        return _require_mt5().market_book_get(broker_symbol)

    def market_book_release(self, symbol: str) -> bool:
        broker_symbol = self._cfg.resolve_symbol(symbol)
        return bool(_require_mt5().market_book_release(broker_symbol))

    def version(self) -> tuple[int, int, str] | None:
        return _require_mt5().version()

    def last_error(self) -> tuple[int, str]:
        return _require_mt5().last_error()

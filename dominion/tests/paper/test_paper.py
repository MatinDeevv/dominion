...
class TestPaperExecutor:
    ...
    def test_pending_order_revalidated_on_trigger(self):
        """Triggered pending orders should be rechecked against current risk state."""
        order = _make_order(order_id="PEND-001")
        order.order_type = OrderType.LIMIT
        order.entry_price = 2349.0

        fill = self.executor.submit_order(order, current_price=2350.0)
        assert fill is None
        assert self.executor.stats["pending_count"] == 1

        self.executor._sentinel.update_equity(10_000.0)  # noqa: SLF001
        self.executor._sentinel.update_equity(8900.0)  # noqa: SLF001

        fills = self.executor.check_pending_orders(current_price=2348.5)

        assert fills == []
        assert order.status == OrderStatus.REJECTED
        assert self.executor.stats["pending_count"] == 0
        assert self.executor.stats["rejection_count"] == 1


def test_execution_enforcer_applies_l1_haircut_once():
    """L1 drawdown should halve size once, not twice."""
    sentinel = _make_sentinel_stack()
    sentinel["clock"].set_simulated_time(datetime(2024, 1, 9, 10, 0, 0, tzinfo=timezone.utc))
    sentinel["core"]._l1_triggered = True  # noqa: SLF001
    ...
    approved, reason, final_size_pct = sentinel["enforcer"].approve_order(proposal)

    assert approved is True
    assert reason == "APPROVED"
    assert final_size_pct == pytest.approx(0.01)

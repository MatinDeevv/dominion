...
SUPPORTED_ACTION_HORIZONS = {"5m": 5, "15m": 15, "60m": 60, "240m": 240}
...
class SignalPublisher:
    ...
    def __init__(
        ...
        action_horizon: str = "60m",
    ) -> None:
        self.model_artifact_id = str(model_artifact_id)
        self.calibrator = calibrator
        self.sizer = sizer
        if action_horizon not in SUPPORTED_ACTION_HORIZONS:
            raise ValueError(
                f"Unsupported action_horizon '{action_horizon}'. "
                f"Expected one of {', '.join(sorted(SUPPORTED_ACTION_HORIZONS))}"
            )
    ...
    def publish(
        ...
        action_direction_probs = _tensor_to_tuple(output.direction_probs(self.action_horizon)[batch_idx])
        action_direction = int(np.argmax(action_direction_probs)) - 1
        action_return_median = _tensor_to_float(output.return_median(self.action_horizon)[batch_idx])
        action_raw_lower, action_raw_upper = output.return_interval(self.action_horizon, alpha=0.8)
        action_return_lower = _tensor_to_float(action_raw_lower[batch_idx])
        action_return_upper = _tensor_to_float(action_raw_upper[batch_idx])
        action_conformal_lower_raw, action_conformal_upper_raw = self.calibrator.predict(
            action_return_lower,
            action_return_upper,
        )
        action_conformal_lower = float(np.asarray(action_conformal_lower_raw).reshape(-1)[0])
        action_conformal_upper = float(np.asarray(action_conformal_upper_raw).reshape(-1)[0])

        direction_prob = action_direction_probs[action_direction + 1]
        dual_source_ratio = float(bar_metadata.get("dual_source_ratio", 0.0))
        kelly_fraction, position_fraction = self.sizer.size(
            direction=action_direction,
            direction_prob=direction_prob,
            conformal_lower=action_conformal_lower,
            conformal_upper=action_conformal_upper,
        ...
        return SignalRecord(
            ...
            action_horizon_minutes=SUPPORTED_ACTION_HORIZONS[self.action_horizon],
            action_direction=action_direction,
            action_direction_probs=action_direction_probs,
            action_return_median=action_return_median,
            action_conformal_lower=action_conformal_lower,
            action_conformal_upper=action_conformal_upper,
        )

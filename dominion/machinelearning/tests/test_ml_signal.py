...
def test_signal_publisher_supports_short_horizon_action_contract() -> None:
    output = _model_output(batch_size=1)
    regime_state = RegimeState.from_probs(torch.tensor([0.1, 0.7, 0.1, 0.1], dtype=torch.float32))
    publisher = SignalPublisher(
        model_artifact_id="model.aphelion.test",
    ...
    assert record.action_horizon_minutes == 5
    assert record.resolved_action_direction() == record.direction_5m
    assert record.action_direction_probs is not None
    assert record.action_return_median is not None
    assert record.action_conformal_lower is not None
    assert record.action_conformal_upper is not None


def test_signal_record_actionability_uses_action_horizon_fields() -> None:
    signal = _signal_record(
        direction_60m=0,
        direction_probs_60m=(0.33, 0.34, 0.33),
    ...
    signal = SignalRecord(
        **{
            **signal.to_dict(),
            ...
            "direction_probs_60m": signal.direction_probs_60m,
            "action_horizon_minutes": 5,
            "action_direction": 1,
            "action_direction_probs": (0.05, 0.05, 0.90),
            "action_return_median": 0.03,
            "action_conformal_lower": 0.01,
            "action_conformal_upper": 0.05,
        }
    )

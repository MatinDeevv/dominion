...
from machinelearning.backtest import BacktestEngine, BacktestReport
...
def test_backtest_uses_action_horizon_direction_when_present() -> None:
    engine = BacktestEngine(spread_bps=0.0, notional=1_000.0)
    signal = _signal(
        0,
        direction=0,
    ...
    signal = SignalRecord(
        **{
            **signal.to_dict(),
            ...
            "action_direction": 1,
            "action_direction_probs": (0.1, 0.1, 0.8),
            "action_return_median": 0.2,
            "action_conformal_lower": 0.1,
            "action_conformal_upper": 0.3,
        }
    )

    result = engine.run([signal], np.array([5.0], dtype=float))

    assert result.metrics is not None
    assert result.metrics.n_trades == 1
    assert result.metrics.action_horizon_minutes == 5

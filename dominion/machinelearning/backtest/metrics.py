"""Trading-relevant metrics for the vectorized ML backtest layer."""
...
class BacktestMetrics:
    """Compact summary of signal quality after costs and sizing."""
...
def compute_metrics(result: "BacktestResult") -> BacktestMetrics:
    ...
    if n_signals == 0:
        return BacktestMetrics(
            ...
            action_horizon_minutes=result.action_horizon_minutes,
        )

    directions = np.fromiter((signal.resolved_action_direction() for signal in result.signals), dtype=np.int64, count=n_signals)
    ...
    predicted_returns = np.fromiter(
        (signal.resolved_action_return_median() for signal in result.signals),
        dtype=np.float64,
        count=n_signals,
    )
    ...
    net_return_bps[valid_position] = result.pnl_series[valid_position] / (result.notional * positions[valid_position])

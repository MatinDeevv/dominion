...
from aphelion.core.execution import apply_enforcement, compute_commission
...
class PaperExecutor:
    ...
    def submit_order(self, order: Order, current_price: float) -> Optional[PaperFill]:
        ...
        # Use shared enforcement pipeline (identical logic to backtest)
        entry_price = current_price if order.order_type == OrderType.MARKET else order.entry_price
        enforced = apply_enforcement(order, entry_price, self._enforcer)

        if enforced is None:
            self._rejection_count += 1
            logger.info(
                "PAPER REJECTED order %s",
                order.order_id,
            )
            self._publish_event("ORDER_REJECTED", {
                "order_id": order.order_id,
                "reason": "ENFORCEMENT_REJECTED",
    ...
    def _fill_market(self, order: Order, current_price: float) -> PaperFill:
        ...
        commission = compute_commission(order.size_lots, self._config.commission_per_lot)
    ...
    def check_pending_orders(self, current_price: float) -> list[PaperFill]:
        ...
        for order in self._pending_orders:
            ...
            if triggered:
                enforced = apply_enforcement(order, order.entry_price, self._enforcer)
                if enforced is None:
                    self._rejection_count += 1
                    logger.info("PAPER REJECTED pending order %s on trigger", order.order_id)
                    self._publish_event("ORDER_REJECTED", {
                        "order_id": order.order_id,
                        "reason": "ENFORCEMENT_REJECTED_ON_TRIGGER",
                        "proposed_by": order.proposed_by,
                    })
                    continue

                fill = self._fill_market(enforced, current_price)
                if fill:
                    fills.append(fill)
            else:
    ...
    def close_position(
        ...
        if pos:
            commission = compute_commission(pos.size_lots, self._config.commission_per_lot)

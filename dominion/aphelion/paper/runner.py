...
from aphelion.risk.titan.gate import TitanGate
...
class PaperRunnerConfig:
    ...
    # ARES governance coordinator (optional - wired by the demo launcher)
    ares: Optional[object] = None


def validate_titan_gate(
    titan: TitanGate,
    required_checks: list[str],
    metrics: dict[str, Any] | None = None,
) -> None:
    """Run TITAN and block startup when configured required checks fail.

    When no metrics are supplied, the paper runner has nothing meaningful to
    evaluate yet, so the gate is skipped with a warning instead of failing on
    ...
    gate_kwargs.setdefault("triggered_by", "paper_startup")

    supplied_metrics = {key: value for key, value in gate_kwargs.items() if key != "triggered_by"}
    if not supplied_metrics:
        logger.warning("TITAN enabled but no metrics were supplied; skipping startup gate")
        return

    report = titan.run_full_gate(**gate_kwargs)
    check_map = {validation.check_name: validation for validation in report.validations}

    failed_checks: list[tuple[str, str]] = []
    for check_name in required_checks:
        result = check_map.get(check_name)
        if result is None:
            logger.warning("TITAN check '%s' not found in report", check_name)
            continue

        if result.passed:
            logger.info(
                "TITAN passed required check %s: actual=%.4f threshold=%.4f",
                check_name,
                result.actual_value,
                result.threshold,
            )
        ...
        failed_checks.append(
            (
                check_name,
                f"actual={result.actual_value:.4f}, threshold={result.threshold:.4f}, message={message}",
            )
        )

    for validation in report.validations:
        if validation.check_name in required_checks:
            continue
        status = "passed" if validation.passed else "failed"
        logger.info(
            "TITAN %s optional check %s: actual=%.4f threshold=%.4f",
            status,
    ...
    if failed_checks:
        failure_text = "\n  ".join(
            f"- {name}: {detail}" for name, detail in failed_checks
        )
        raise RuntimeError(
            "TITAN gate blocked session start. Required checks failed:\n  "
            f"{failure_text}"
        )

    logger.info("TITAN gate passed all %d required checks", len(required_checks))
...
class PaperRunner:
    ...
    async def run(self) -> PaperSessionResult:
        ...
        # 4. Validate TITAN gate before session creation
        if self._config.session_config.titan_enabled:
            validate_titan_gate(
                TitanGate(),
                self._config.session_config.get_titan_required_checks(),
                metrics=self._config.session_config.titan_metrics,
            )

        # 5. Create and run paper session

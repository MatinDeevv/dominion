...
class PaperSessionConfig:
    ...
    # TITAN startup gate
    titan_enabled: bool = True
    titan_required_checks: list[str] | None = None
    titan_metrics: dict[str, object] = field(default_factory=dict)

    def get_titan_required_checks(self) -> list[str]:
        """Return the TITAN checks that must pass before startup."""
        if self.titan_required_checks is not None:
            return list(self.titan_required_checks)
        return [
            "min_sharpe_ratio",
            "max_drawdown",
            "min_profit_factor",
...
class PaperSession:
    ...
    def __init__(
        ...
        # ── Feature engine ────────────────────────────────────────────────
        self._feature_engine = LegacyFeatureEngine(self._data_layer, self._clock)
    ...
    def _init_hydra(self) -> None:
        ...
        if self._config.hydra_checkpoint:
            try:
                ...
                logger.error("HYDRA unavailable: %s", exc)
                self._ledger.log_error("HYDRA unavailable", {"error": str(exc)})
    ...
    def _govern_orders_with_ares(self, orders: list[Order], bar_ts: datetime | None) -> list[Order]:
        """Apply ARES governance to outbound orders.

        The current ARES contract only scores one vote at a time, so multi-order
        strategy outputs are reduced to the first order after logging a warning.
        """
        if not orders or self._ares is None or not _HAS_ARES:
            return orders

        if len(orders) > 1:
            logger.warning(
                "Strategy produced %d orders; ARES only governs first. Submitting only orders[0].",
                len(orders),
            )
            orders = [orders[0]]
        ...
        if not aggregated.is_actionable:
            logger.warning(
                "ARES rejected order: direction=%s confidence=%.2f veto=%s reason=%s",
                direction,
                confidence,
                aggregated.vetoed,
                aggregated.veto_reason,
            )
        ...
        logger.info(
            "ARES approved order: direction=%s confidence=%.2f consensus_score=%.2f",
            direction,
            confidence,
    ...
    def _process_bar(self, bar: Bar) -> None:
        ...
        self._data_layer.ingest_completed_bar(bar)
        ...
        if self._bar_count >= self._config.warmup_bars and self._strategy is not None:
            try:
                ...
                orders = self._govern_orders_with_ares(orders, bar_ts)

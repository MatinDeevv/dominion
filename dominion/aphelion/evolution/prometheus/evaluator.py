...
NOTE: GenomeStrategy intentionally does NOT create Order objects directly.
It delegates to ``build_order_from_signal`` in ``aphelion.core.execution``
so that all order-construction logic is centralised and the evolution
module stays within its containment boundary (search/optimisation only).
"""
...
class GenomeStrategy:
    ...
    def __call__(
        ...
        if portfolio.get_open_position_count() >= max_pos:
            return []
        ...
        # ── Delegate order creation to canonical factory ─────────────────

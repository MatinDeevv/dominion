...
def _regularized_incomplete_beta(x: float, a: float, b: float, max_iter: int = 200) -> float:
    ...
    cf = _beta_continued_fraction(x, a, b, max_iter=max_iter)
    value = front * cf
...
def _beta_continued_fraction(x: float, a: float, b: float, max_iter: int = 200) -> float:
    ...
    This follows the standard Numerical Recipes form and avoids the
    overwritten odd-step numerator bug that previously skewed p-values.
    """
    tiny = 1e-30
    qab = a + b
    qap = a + 1.0

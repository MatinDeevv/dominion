...
Architecture
────────────
aphelion.tui.launcher  ->  AphelionController(config)  ->  TUI
...
class AphelionController:
    """Single owner of ALL background work — session, training, forge, backtest.
        ...
        Created in the package TUI launcher and passed into the TUI.
    ...
    def _get_or_create_event_loop() -> asyncio.AbstractEventLoop:
        """Return the current loop, creating one when called from sync code."""
        try:
    ...
    def start_session(self, mode: str = "paper") -> None:
        ...
        if state:
            ...
            state.feed_mode = str(mode).upper()
        try:
    ...
    def stop_session(self) -> None:
        ...
        if self._session_ctrl.is_active:
            try:
                loop = self._get_or_create_event_loop()
                if loop.is_running():
                    loop.create_task(self._session_ctrl.stop_session())
                else:
